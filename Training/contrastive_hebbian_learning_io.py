"""Contrastive Hebbian learning (daydreaming) training for Model A — input/output (IO) relation task.
...
Unlike the target-phase-retrieval variant, `CHL_training_hidden` here starts
from a random (near-zero) interaction matrix rather than a Hebbian guess
derived from the target memories, and supports continuing training from a
previous result via `chi_prev`/`miu_prev`.
"""
...
def CHL_training_hidden(target_memories, n_species, n_epochs, clamped=None,
    ...
    """Train chi/miu via contrastive Hebbian learning for an input/output relation.

    `chi_initial` is a random (near-zero), symmetric interaction matrix with
    the diagonal and the last row/column (the solvent species) fixed at zero
    -- unlike the target-phase-retrieval variant, which uses a Hebbian guess
    derived from the target memories.

    If `chi_prev`/`miu_prev` are given (e.g. from a previous call), training
    continues from those values instead of this random initial guess.
    """
    if seed is None:
    ...
    # random symmetric initial interaction matrix; solvent species (last) fixed at zero
    chi_initial = rng.uniform(low=-0.0, high=0.0, size=(n_species, n_species))
